## 目录
- Q0: Hypervisor 安装启动中出现 /OS/1 I/O access blocked. 影响安装吗
...
1、在Win10系统下按Win+R按键，启动 【命令行(管理员模式)】。注意：一定要是管理员模式。
...
A11:此问题即是分区资源分配的不合理导致。需检查并确认：

1.  硬盘最后的区域属于Unallocated状态且不小于1GB，
    1.  划分出来的区域不是Windows下的扩展分区
        1.  通过DiskGenius软件检查并确认了系统的分区情况合理
...
1\. 在Win 10 2016上安装Hypervisor （AS4.7 AR:H4.73 )正常使用，但偶发出现网口丢失问题

2\. 升级高版本Hypervisor (AS4.12 AR:B4.93)，无法安装，提示The original boot loader could not be backuped

3\. 重新安装旧版本 (AS4.7 AR:H4.73)，能够正常安装

4\. 使用更新的版本(AS4.12 AR:E4.93)，能够正常安装
